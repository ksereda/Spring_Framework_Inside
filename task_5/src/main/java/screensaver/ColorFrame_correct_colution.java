 ...
 * Верное решение
 *
 * Если у нас есть потребность обратиться к контексту то мы не можем метод getColor() здесь прописать, тюкю мы не хотим здесь содержать контекст.
 * => метод и класс делаем abstract.
 * А там, где мы прописываем из него бин (в java конфиге),
 *
 */
